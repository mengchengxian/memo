        ...
        <div class="container">
            <div class="navbar-header">
                <a class="navbar-brand">
                    <img height="40" src="../assets/imgs/logo.png" />➕
                    <img height="40"
...
const { data } = useStore()
const { aHelper, filterCateId, transMemo, isShow } = storeToRefs(data)

function showAdd() {
    transMemo.value = new ItemData(-1, 0);
    isShow.value = true;
}

function doFilter(cid: number): number {
    if (cid == -1) {
        return aHelper.value.memoList.length;
    } else {
        return aHelper.value.memoList.filter((ele: any) => {
            return ele.categoryId == cid;
        }).length;
    }
}

function doFilterByCateId(cid: number): void {
    filterCateId.value = cid;
}
</script>
<style scoped>
.navbar-brand>img {
